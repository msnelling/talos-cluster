...
**The same CronJob is the only path from "has TrueHD" to "has a DD+ Atmos file".** After selection it lists every film and episode with TrueHD/DTS:X and no E-AC-3 Atmos track (only a different file gives those height), then moves films tagged `atmos-upgrade` in Radarr onto the `Apple TV Atmos (2160p)` profile and searches each once. The tag is the opt-in: nothing is moved without it, so tagging a disc remux is the decision to let Radarr replace it with a WEB-DL. The tag is created empty on the first applied run so it is offered in Radarr's UI. Moves are capped by `radarr.maxMoves` per run (`MAX_MOVES=N task utility:plex-audio-tracks -- --apply` for a bulk tagging session). Episodes are report-only: Sonarr profiles are per-series, so moving a series puts every one of its files up for replacement, not just the listed ones — that is a per-show decision, made by hand. The Sonarr profile for that is `Apple TV Atmos (1080p/2160p)` (TRaSH WEB-2160p Combined: WEB 1080p below WEB 2160p, WEB only, lossless/DTS codecs scored −10000); assign it to a series in Sonarr's UI and search. The Radarr key is read from `recyclarr-api-keys`.
